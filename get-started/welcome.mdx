...
You install Sparkle once — as an IDE extension or through the CLI — and it works inside the tools your team already uses: Cursor, Windsurf, VS Code, Claude Code, and Codex.
...
<Steps>
  ...
  <Step title="Guardrails during generation">
    When an AI coding task begins, Sparkle provides repository-specific rules that guide the agent while it plans, generates, and modifies code.
  </Step>
...
Read [How guardrails work](/guardrails/overview) for the full picture.
...
Both paths connect to the same Sparkle workspace and apply the same guardrails. The extension bundles the CLI, so most developers only need one install.
