...
| `sparkle logout` | Sign out or remove config for a specific tool |
| `sparkle-hooks` | Hook handler invoked by your IDE (installed by setup) |
...
Hook handler invoked by your IDE on prompts, MCP calls, file edits, and session events. Runs silently as a telemetry sidecar.

You do not run this command directly. `sparkle setup` installs and wires it up for you.
