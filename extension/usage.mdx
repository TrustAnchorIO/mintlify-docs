...
<Steps>
  ...
  <Step title="Use AI features">
    Use your IDE's AI chat, agent, or inline edit features as you normally would. Sparkle applies guardrails on every prompt without changing your workflow.
  </Step>
</Steps>
...
1. Identifies the target repository from your open workspace
...
Guardrails can cover standards such as OWASP, PCI-DSS, and custom organization policies. The agent uses them during planning and code generation so issues are caught at the source rather than in review or scanning.
...
| **Sparkle: Show MCP Status** | Check whether the Sparkle MCP server is connected |
| **Sparkle: Diagnose** | Run diagnostics when something is not working |
| **Sparkle: Reset All Data** | Clear local Sparkle extension data and sign out |
...
<Steps>
  <Step title="Check MCP status">
    Run **Sparkle: Show MCP Status** from the Command Palette. The Sparkle MCP server should show as connected.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Troubleshooting" href="/help/troubleshooting" icon="wrench">
    Fix MCP connection issues and skipped guardrail calls.
  </Card>
</CardGroup>
